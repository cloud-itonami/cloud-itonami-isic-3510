(ns open-denki.app
  "etzhayyim-project-open-denki worker appview — reagent + re-frame, view built
  ...
  Three fields are updated, not simplified, and each reason is recorded
  here to stay honest about what this migration changed:

  - `:app/route-count` and `:app/routes` were `0` / `[]` in the Svelte
    source, which never matched this repo's own `worker/wrangler.jsonc`
    (one `routes` entry, `open-denki.etzhayyim.com/*`, has always been
    declared there). This port reports what wrangler actually declares,
    not the stale literal the generated Svelte page shipped with.
  - `:app/vars` was `[]` in the Svelte source, which likewise never
    matched `worker/wrangler.jsonc`'s four declared `vars` keys
    (`AGENTGATEWAY_MCP_ROUTER_URL`, `APP_FRAMEWORK`, `APP_HANDLE`,
    `PRIMARY_DID`). Same fix, same reason, alphabetized like the wrangler
    config's own keys.
  - `:app/xrpc?` is now false. This migration's wrangler.jsonc drops
    ...
    Worker source in this repo (`src/app.ts` nor the moved
    `src/xrpc-mcp-router-proxy.ts`) calls `env.ASSETS.fetch`, so the XRPC
    proxy route this page used to advertise as enabled no longer deploys.
    The proxy handler itself is preserved byte-for-byte at
  ...
  `:app/relative-path` also changes value (it now names this file, not
  the deleted Svelte one), but this is a copy-forward, not a correction:
  the field always names 'the file this data structure lives in', so a
  file move updates it by definition, the same way `:app/xrpc?` and the
  route/var facts above are corrections of stale content, not renames.
  ...
  components or ext-rules change) with:
    ...
    (spit \"public/index.html\"
          (page/->page {:title \"etzhayyim-project-open-denki\"
                         :lang \"ja\"
                         :description \"etzhayyim-project-open-denki worker — smart-grid network design + operations appview (reagent + re-frame + jp-go-dds).\"
                         :css (slurp (io/resource \"jp_go_dds/dds.css\"))}
                        [:div {:id \"app\"} \"etzhayyim-project-open-denki loading…\"]
                        [:noscript \"etzhayyim-project-open-denki requires JavaScript.\"]
...
(def default-db
  {:app/title "Worker"
   :app/project "etzhayyim-project-open-denki"
   :app/name "worker"
   :app/kind "worker"
   :app/route-count 1
   :app/routes ["open-denki.etzhayyim.com/*"]
   :app/vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_FRAMEWORK" "APP_HANDLE" "PRIMARY_DID"]
   :app/xrpc? false
   :app/relative-path "worker/cljs/src/open_denki/app.cljs"})
