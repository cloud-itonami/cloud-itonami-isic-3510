(ns grid.operation
  "OperationActor -- one distribution-utility operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor (Grid
  Distribution Advisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Grid Transmission Governor
  ...
  One graph run = one distribution-utility operation (intake -> advise
  ...
  decision to a human operator (the distribution utility). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/disconnect-service` ALWAYS reaches this node when the
  governor is clean (never auto, at any phase); `:actuation/provision-
  service` reaches it only when the governor found it high-stakes
  (over the capacity threshold) or low-confidence -- see `grid.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [grid.gridadvisor :as gridadvisor]
            [grid.governor :as governor]
            [grid.phase :as phase]
            [grid.store :as store]))
...
(defn build
  ...
  `grid.store/Store`).
  opts:
    :advisor      -- a `grid.gridadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (gridadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Grid Distribution Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (gridadvisor/-advise advisor store request)]
            {:proposal p :audit [(gridadvisor/trace request p)]})))

      ;; Grid Transmission Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
